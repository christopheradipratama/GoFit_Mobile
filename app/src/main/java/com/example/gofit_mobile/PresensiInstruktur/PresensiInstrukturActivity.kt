...
import com.example.gofit_mobile.api.PresensiInstrukturApi
import com.example.gofit_mobile.databinding.ActivityPresensiInstrukturBinding
import com.example.gofit_mobile.model.PresensiInstrukturModel
import com.example.gofit_mobile.model.PresensiInstrukturModel2
    ...
    private fun allData() {
        ...
        val stringRequest: StringRequest = object :
            StringRequest(Method.GET, PresensiInstrukturApi.GET_ALL_URL, Response.Listener { response ->
                ...
                var schedule = arrayListOf<PresensiInstrukturModel>()
                var id : Int = jo.getJSONArray("data").length()

                for(i in 0 until id) {
                    var data = PresensiInstrukturModel(
                ...
                var data_array: Array<PresensiInstrukturModel> = schedule.toTypedArray()
...
//                    Toast.makeText(this@ScheduleInstructorActivity, "Data Berhasil Diambil!", Toast.LENGTH_SHORT).show()
    ...
    fun store(ID_INSTRUKTUR: Int, TANGGAL_MENGAJAR: String){
        val presence = PresensiInstrukturModel2(
            ID_INSTRUKTUR,
            TANGGAL_MENGAJAR
        )

        val stringRequest: StringRequest =
            object : StringRequest(Request.Method.POST, PresensiInstrukturApi.ADD_URL, Response.Listener { response ->
                val gson = Gson()
                var presence_data = gson.fromJson(response, PresensiInstrukturModel2::class.java)
